...
def test_no_spinning_friction(
    ...
    ball.angular_velocity = DVector3(0, 1, 0)
...
def test_spinning_friction() -> None:
    ...
    def reset_world():
        ...
        ball_1.spinning_friction = .5
        ball_1.transform = DMatrix4(1).translate(DVector3(5, 1.1, -5))
        ball_1.angular_velocity = DVector3(0, 1, 0)

        ball_2 = Body(
            1,
            Sphere(DVector3(0), 1), world=w, type=BodyType.DYNAMIC
        )
        ball_2.spinning_friction = .5
        ball_2.transform = DMatrix4(1).translate(DVector3(-5, 1.1, 5))
        ball_2.angular_velocity = DVector3(0, 1, 0)
    ...
    assert is_close(ball_1.angular_velocity.x, 0)
    assert is_close(ball_1.angular_velocity.y, 0.38536720634579347)
    assert is_close(ball_1.angular_velocity.z, 0)

    assert is_close(ball_2.angular_velocity.x, 0)
    assert is_close(ball_2.angular_velocity.y, 0.38536720634579347)
    ...
    assert is_close(ball_1.angular_velocity.x, 0)
    assert is_close(ball_1.angular_velocity.y, 1)
    assert is_close(ball_1.angular_velocity.z, 0)

    assert is_close(ball_2.angular_velocity.x, 0)
    assert is_close(ball_2.angular_velocity.y, 1)
    assert is_close(ball_2.angular_velocity.z, 0)

...
def test_linear_velocity(linear_velocity: DVector3) -> None:
    w = World(timedelta(seconds=.1))
    b = Body(1, Sphere(DVector3(0), 1), world=w)
    ...
    assert all(
        ...
        for r, e in zip(b.transform, DMatrix4(1).translate(linear_velocity))
    )
...
def test_linear_velocity_non_dynamic(body_type: BodyType) -> None:
    ...
    b.linear_velocity = DVector3(1, 2, 3)
...
def test_no_linear_damping() -> None:
    ...
    w.simulate(timedelta(seconds=10))
    assert b.linear_velocity == DVector3(1, 2, 3)


def test_linear_damping() -> None:
    w = World(timedelta(seconds=1))
    b = Body(1, Sphere(DVector3(0), 1), world=w)
    b.linear_velocity = DVector3(10, 10, 10)
    b.linear_damping = .5

    w.simulate(timedelta(seconds=1))
    assert b.linear_velocity == DVector3(5, 5, 5)
    w.simulate(timedelta(seconds=1))
    assert b.linear_velocity == DVector3(2.5, 2.5, 2.5)
    w.simulate(timedelta(seconds=1))
    assert b.linear_velocity == DVector3(1.25, 1.25, 1.25)
...
def test_linear_damping_static() -> None:
    ...
    b.linear_velocity = DVector3(pi, pi, pi)
    b.linear_damping = .5

    w.simulate(timedelta(seconds=10))
    assert b.linear_velocity == DVector3(0, 0, 0)
...
def test_linear_velocity_switch_to_kinematic() -> None:
    w = World(timedelta(seconds=.1))
    b = Body(1, Sphere(DVector3(0), 1), world=w)
    b.linear_velocity = DVector3(pi, 0, 0)
    b.linear_damping = .5

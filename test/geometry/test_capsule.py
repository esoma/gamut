...
def test_hash() -> None:
    c1 = Capsule(Vector3(1, 2, 3), 4, 5, rotation=Quaternion(6, 7, 8, 9))
    c2 = Capsule(Vector3(1, 2, 3), 4, 5, rotation=Quaternion(6, 7, 8, 9))
    assert hash(c1) != hash(c2)


def test_repr() -> None:
    capsule = Capsule(Vector3(1, 2, 3), 4, 5, rotation=Quaternion(6, 7, 8, 9))
    assert (
        repr(capsule) ==
...
def test_invalid_rotation(rotation: Any) -> None:
    with pytest.raises(TypeError) as excinfo:
        Capsule(Vector3(0), 0, 0, rotation=rotation)
...
def test_equal() -> None:
    assert Capsule(Vector3(0), 0, 0) == Capsule(Vector3(0), 0, 0)
    assert Capsule(Vector3(0), 0, 0) != Capsule(Vector3(1, 0, 0), 0, 0)
    assert Capsule(Vector3(0), 0, 0) != Capsule(Vector3(0, 1, 0), 0, 0)
    assert Capsule(Vector3(0), 0, 0) != Capsule(Vector3(0, 0, 1), 0, 0)
    assert Capsule(Vector3(0), 0, 0) != Capsule(Vector3(0), 1, 0)
    assert Capsule(Vector3(0), 0, 0) != Capsule(Vector3(0), 0, 1)
    assert Capsule(Vector3(0), 0, 0) != object()

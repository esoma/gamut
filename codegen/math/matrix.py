...
def generate_matrix_files(build_dir: Path) -> Generator[str, None, None]:
    ...
    for r in range(2, 5):
        for c in range(2, 5):
            type = generate_matrix_file(b,
                'double', r, c, f'DMatrix{r}x{c}', 'd',
                f'DVector{c}', f'DVector{r}'
            )
            types.append(type)
            yield type[0]
            type = generate_matrix_file(b,
                'float', r, c, f'FMatrix{r}x{c}', 'f',
                f'FVector{c}', f'FVector{r}'
            )
            types.append(type)
            yield type[0]
...
def generate_matrix_type_file(
    build_dir: Path,
    types: Sequence[tuple[str, int, int, str]]
) -> None:

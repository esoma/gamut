...
class LineSegment3d(Generic[T]):
    ...
    def _get_point_projection_time(self, point: T) -> float:
        ap = point - self._a
    ...
    def project_point(
        ...
        *,
        ...
        t = self._get_point_projection_time(point)
        if clamped:
            t = max(min(t, 1), 0)
        return self._a + t * self._slope
    ...
    def where_intersected_by_line_segment(
        ...
        *,
        ...
        if det == 0:
            ...
            oat = self._get_point_projection_time(other._a)
            ...
            obt = self._get_point_projection_time(other._b)
            obp = self.get_point_from_a_to_b(obt)
            intersections = [
                self.project_point(op, clamped=True)
                for op in (oap, obp)
                if self.distance_to_point(op) <= tolerance
            ]
